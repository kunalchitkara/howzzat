...
export default async function MatchScorecardPage({
  ...
  let matchUrlRef = matchId;
  try {
    const match = await getMatch(matchId);
    if (shouldRedirectToMatchSlug(matchId, match)) {
      redirect(`/match/${match.slug}`);
    }
    matchUrlRef = matchPublicRef(match);
    const [scorecard] = await Promise.all([buildMatchScorecardView(match.id)]);
    data = scorecard;
    ...
    return (
      <main style={{ background: "#eef2f7", minHeight: "100vh", padding: 24 }}>
        <div style={{ maxWidth: 480, margin: "0 auto", textAlign: "center" }}>
          ...
          <LiveScorePoll matchId={matchUrlRef} initialStatus={matchStatus} />
          <p style={{ marginTop: 16 }}>
            <Link href={`/match/${matchUrlRef}/score`} style={{ color: "var(--md)", marginRight: 12 }}>
              Open scorer
            </Link>
    ...
    <main style={{ background: "#eef2f7", minHeight: "100vh", padding: "0 0 24px" }}>
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "12px 16px 0" }}>
        <div style={{ textAlign: "right", marginBottom: 8 }}>
          <Link href={`/match/${matchUrlRef}/score`} style={{ color: "var(--md)", fontSize: "0.85rem", fontWeight: 600 }}>
            Open scorer →
          </Link>
        </div>
        <LiveScorePoll matchId={matchUrlRef} initialStatus={matchStatus} />
